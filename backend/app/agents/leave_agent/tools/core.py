...
def _leave_days_in_month(leave: LeaveRequest, *, year: int, month: int) -> float:
    """Count only the days of this leave that fall within the queried month.

    The query that selects `approved_leaves` only tests whether the request
    *overlaps* the month (start_date <= month_end and end_date >= month_start)
    -- it can match a leave that spans a month boundary. Using the request's
    full `total_days` for every month it overlaps double- (or triple-)
    counts it. This instead intersects the leave's date range with the
    queried month and counts working days (Mon-Fri) only within that
    intersection, so a 6-day leave crossing a month boundary is split
    correctly instead of being charged 6 days in both months.
    """
...
def calculate_lop(db: Session, *, employee_id: UUID, month: int, year: int) -> LOPResult:
    ...
    for leave in approved_leaves:
        days = _leave_days_in_month(leave, year=year, month=month)
